...
/**
 * SPHybridDriverDeterm.java
 * A driven program is used to submit the single source shortest 
 * distance computation job using the STYLE.Hybrid model.
 ...
 * @version 0.1
 */
public class SSSPHybridDriverDeterm {
	
	public static void main(String[] args) throws Exception {
		//check parameters
		if (args.length < 7) {
			StringBuffer sb = 
				new StringBuffer("\nUsage of Hybrid-based Shortest Distance (deterministic):");
			sb.append("\n(*)required parameter");
			sb.append("\n[*]optional parameter");
		...
		BSPJob bsp = new BSPJob(conf, SSSPHybridDriverDeterm.class);
		bsp.setJobName("Single Source Shortest Distance");
		bsp.setPriority(Constants.PRIORITY.NORMAL);

		bsp.setBspClass(SPBSPDeterm.class);
		bsp.setUserToolClass(SPUserToolDeterm.class);
		...
		bsp.setInt(SPBSPDeterm.SOURCE, Integer.valueOf(args[6]));
