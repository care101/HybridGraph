...
/**
 * SemiCluster-Driver.java
 * A driven program is used to submit the semi-cluster job.
 * 
 * @author zhigang wang
 * @version 0.2
 */
public class SCPullDriver {
	
	public static void main(String[] args) throws Exception {
		//check the input parameters
		if (args.length != 7) {
			StringBuffer sb = 
				new StringBuffer("the semi-cluster job must be given arguments(7):");
		...
		BSPJob bsp = new BSPJob(conf, SCPullDriver.class);
		bsp.setJobName("Semi-Cluster");
		bsp.setBspClass(SCBSP.class);
		bsp.setPriority(Constants.PRIORITY.NORMAL);
		bsp.setUserToolClass(SCUserTool.class);
