 ...
 */
public class PageRankUserTool extends UserTool<Double, Integer, Double, EdgeSet> {
	public static final Log LOG = LogFactory.getLog(PageRankUserTool.class);
	
	public class PRGraphRecord extends GraphRecord<Double, Integer, Double, EdgeSet> {
		...
		@Override
		public ArrayList<GraphRecord<Double, Integer, Double, EdgeSet>> 
			...
			ArrayList<GraphRecord<Double, Integer, Double, EdgeSet>> result = 
				new ArrayList<GraphRecord<Double, Integer, Double, EdgeSet>>();
			for (dstTid = 0; dstTid < tNum; dstTid++) {
		...
		public MsgRecord<Double>[] getMsg(int iteStyle) {
			...
			PRMsgRecord[] result = new PRMsgRecord[eids.length];
			for (int i = 0; i < eids.length; i++) {
				result[i] = new PRMsgRecord();
				result[i].initialize(this.verId, eids[i], this.verValue);
			}
			return result;
		}
		...
		public Double getFinalValue() {
			return this.graphInfo.getEdgeNum()==0? 
					this.verValue:this.verValue*this.graphInfo.getEdgeNum();
		}
	...
	@Override
	public GraphRecord<Double, Integer, Double, EdgeSet> getGraphRecord() {
